...
def format_plural(unit):
    """
    Returns 's' if the unit is not 1, otherwise returns an empty string.

    Examples:
    >>> days = 5
    >>> f"{days} day{format_plural(days)}"
    '5 days'
    >>> days = 1
    >>> f"{days} day{format_plural(days)}"
    '1 day'
...
def commit_counter(comment_size):
    ...
    total_commits = 0
    filename = 'cache/' + hashlib.sha256(
        USER_NAME.encode('utf-8')).hexdigest() + '.txt'  # Use the same filename as cache_builder
    with open(filename, 'r') as f:
        data = f.readlines()
    cache_comment = data[:comment_size]  # save the comment block
    data = data[comment_size:]  # remove those lines
...
if __name__ == '__main__':
    ...
    print(f"Star Data: {star_data}")
    print(f"Repo Data: {repo_data}")
    print(f"Contrib Data: {contrib_data}")
    print(f"Follower Data: {follower_data}")
    ...
    # move cursor to override 'Calculation times:' with 'Total function time:' and the total function time,
    # then move cursor back
